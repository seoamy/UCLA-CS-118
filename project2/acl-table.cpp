...
ACLTable::lookup(uint32_t srcIp, uint32_t dstIp, uint8_t protocol, uint16_t srcPort, uint16_t dstPort) const
{
  ...
  ACLTableEntry* largest_priority_match = nullptr;
  while (itr != m_entries.end()) {
    // compare IPs, ports, and protocol
    if (((itr->src & itr->srcMask) == (srcIp & itr->srcMask))
        && ((itr->dest & itr->destMask) == (dstIp & itr->destMask))
        && ((itr->srcPort & itr->srcPortMask) == (srcPort & itr->srcPortMask))
        && ((itr->destPort & itr->destPortMask) == (dstPort & itr->destPortMask))
        && ((itr->protocol & itr->protocolMask) == (protocol & itr->protocolMask))) {
          // compare priority
          if (largest_priority_match == nullptr || largest_priority_match->priority < itr->priority) {
              largest_priority_match = (ACLTableEntry*) &*itr;
          }
    }
    itr++;
  }
  ...
  if (largest_priority_match != nullptr) {
    return *largest_priority_match;
  }

  else {
    throw std::runtime_error("No ACL entry found.");
  }
}
