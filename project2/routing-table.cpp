...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  // FILL THIS IN
  RoutingTableEntry* longest_prefix_match = NULL;
  auto itr = m_entries.begin();
  while (itr != m_entries.end()) {
    uint32_t ip_mask = ip & itr->mask;
    uint32_t dest_mask = itr->dest & itr->mask;
    if (ip_mask == dest_mask) { // match prefix
      if((longest_prefix_match == NULL) || itr->mask >= longest_prefix_match->mask) {
        longest_prefix_match = (RoutingTableEntry*) &*itr;
      }
    }
    itr++;
  }
